...
class UXAnalyzer:
    ...
    def __init__(self, image_path: str, use_ocr: bool = True):
        if not HAS_CV2:
            raise ImportError(
                f"Missing required dependencies: {_IMPORT_ERROR}\n"
    ...
    def _get_region_color(self, region_image: Any) -> Dict[str, str]:
        ...
        return {
            "dominant": rgb_to_hex(dominant[:3]),
            "average": rgb_to_hex(avg_color[:3])
        }
